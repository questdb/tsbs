#!/usr/bin/env python3
"""ILP-only matrix, for builds without QWP (the released image)."""
...
def run(label, data, extra, workers):
    ...
    return {"label": label, "loader_rows_s": EXPECTED / loader_secs,
    ...
    for r in range(rounds):
        ...
        for label, data, extra in configs:
            ...
            if res:
                res["round"] = r + 1
                results.append(res)
    with open("/home/ubuntu/ingest-results-release.json", "w") as fh:
        json.dump(results, fh, indent=2)
    print("saved /home/ubuntu/ingest-results-release.json")
